...
def traffic_light():
    threading.Timer(0.1,traffic_light).start()
    if dropped_vehicle.is_at_traffic_light():
        ...
        if traffic_light.get_state() == carla.TrafficLightState.Red:
            ...
            dropped_vehicle.apply_control(carla.VehicleLightState(carla.VehicleLightState.Brake))

        if traffic_light.get_state == carla.TrafficLightState.Yellow:
            RewTime = traffic_light.get_red_time
            print(RewTime)
            print(traffic_light.get_state())
